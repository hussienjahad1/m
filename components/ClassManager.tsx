...
export default function ClassManager({ classes, onUpdateClasses, onSelectClass, currentUser }: ClassManagerProps): React.ReactNode {
        ...
        if (editingClass.id) { // Editing existing class
            const updatedClasses = classes.map(c => 
                ...
                ? { ...c, 
                    ...
                    subjects: editingClass.subjects || c.subjects,
    ...
    const addStudentsToClass = (newStudents: Student[]) => {
        ...
        const updatedClasses = classes.map(c => {
            if (c.id === targetClassId) {
                const existingStudents = c.students || [];
    ...
    const handleAddSubject = () => {
        ...
        setEditingClass(prev => ({ ...prev, subjects: [...(prev?.subjects || []), newSubject] }));
        setNewSubjectName('');
    };
    ...
    const handleSaveSubject = (subjectId: string) => {
        ...
        setEditingClass(prev => {
            ...
            return { ...prev, subjects: prev.subjects.map(s => s.id === subjectId ? { ...s, name: editingSubjectName.trim() } : s) };
        });
    ...
    return (
        <div className="bg-white p-8 rounded-xl shadow-lg">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <select value={editingClass?.stage || ''} onChange={(e) => handleStageChange(e.target.value)} className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-cyan-500 focus:border-cyan-500 sm:text-sm rounded-md">
                                ...
                                <div>
                                    ...
                                    <input type="text" value={editingClass?.section || ''} onChange={(e) => setEditingClass({...editingClass, section: e.target.value})} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-cyan-500 focus:border-cyan-500" placeholder="مثال: أ, ب, ج" />
                                </div>
                                {MINISTERIAL_STAGES.includes(editingClass?.stage || '') && (
                                    <>
                                        <div>
                                            ...
                                            <input type="number" value={editingClass?.ministerialDecisionPoints ?? 5} onChange={(e) => setEditingClass({...editingClass, ministerialDecisionPoints: parseInt(e.target.value, 10) || 0})} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-cyan-500 focus:border-cyan-500" />
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700">عدد دروس الاكمال الخاصة بالمرحلة</label>
                                            <input type="number" value={editingClass?.ministerialSupplementarySubjects ?? 3} onChange={(e) => setEditingClass({...editingClass, ministerialSupplementarySubjects: parseInt(e.target.value, 10) || 0})} className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-cyan-500 focus:border-cyan-500" />
                                        </div>
                                    </>
                                )}
                            </div>
            ...
            {isStudentModalOpen && selectedClassForStudentAdd && (
