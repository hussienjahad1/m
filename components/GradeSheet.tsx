...
export default function GradeSheet({ classData, settings, onUpdateClasses }: GradeSheetProps): React.ReactNode {
    ...
    const handleGradeChange = useCallback((studentId: string, subjectName: string, field: keyof SubjectGrade, value: number | null) => {
        const newStudents = students.map(s => {
            if (s.id === studentId) {
                const newGrades = { ...s.grades, [subjectName]: { ...(s.grades[subjectName] || DEFAULT_SUBJECT_GRADE), [field]: value } };
                return { ...s, grades: newGrades };
            }
            return s;
        });
        onUpdateClasses([{...classData, students: newStudents}]);
    }, [classData, students, onUpdateClasses]);
    ...
    const handleAddStudent = useCallback(() => {
        ...
        onUpdateClasses([{ ...classData, students: updatedStudents }]);
        setIsAddStudentModalOpen(false);
        setNewStudentData({ name: '', registrationId: '', birthDate: '', examId: '' });
    }, [newStudentData, classData, students, onUpdateClasses]);
